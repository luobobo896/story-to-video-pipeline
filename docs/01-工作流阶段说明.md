...
| **依赖** | **Skill**：`ffmpeg-skill`（42 个剪辑/交付工具，本项目不自造 ffmpeg 命令）+ `higgsfield-generate`；**本地**：`ffmpeg-full 9.0.1`（**必须**——普通 ffmpeg 不含字幕/文字滤镜）、`python3`、`ImageMagick`。调用规范见 [rules/FF-剪辑与交付.md](../rules/FF-剪辑与交付.md) |
| **跑偏检查** | ① `check.py --platform tiktok` 必须 **0 failed**（FF-006）；② 复盘 `look.py` 出的 QA 图，9:16 分发版主体未被裁（FF-008 / FF-009）；③ 无黑帧、无音画不同步；④ 时长符合单集目标；⑤ 抽 5 帧与风格锚图比对色温偏差 |
...
python3 $S/scripts/look.py project/work/edit/EP01_9x16_v003.mp4 --tiles 3x2
```
