...
5. **额度与套餐**：`higgsfield account status` 查余额。**Soul 人脸训练需要 Basic 以上套餐（本项目未购买，因此全部角色走定妆图路线）**；其余生成能力按额度消耗。
...
| S2 设定出图 | Higgsfield `nano_banana_2` / `gpt_image_2_5` —— 与后续视频同平台，参考图可直接复用 | ComfyUI + 开源文生图 —— 免费、可训 LoRA；缺点是要自建环境、质量波动大 | 即梦 / Midjourney —— 质量高；缺点是资产跨平台搬运会断一致性链 |
